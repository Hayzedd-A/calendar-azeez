...
function getLocalStorageData(yearStorage) {
  ...
  $.each(alltodoList, function (index, value) {
    ...
    todoListDate.push(
      alltodoList[index].reminderDate.split(" ")[0].slice(0, -1)
    );
...
function highlightUserDate() {
  ...
  function linearRotate() {
    $(".userDate").css(
      "background-image",
      "linear-gradient(" + angle + "deg, #423ca3, #954545, transparent)"
    );
    angle += 30;
    if (angle == 330) {
      angle = 0;
...
function monthSpanner(yearData) {
  $.each(yearData, function (index, value) {
    ...
    $("#" + index + " .dates>div:nth-of-type(8)").addClass(
      "span span-" + value
    );
  });
}
  ...
  $.each(todoListMonths, function (index, value) {
    ...
    $(".todoList ." + value).append(
      '<div class="eventContent ' +
        ...
        todoListEvent[index] +
        ": " +
        todoListReminder[index] +
        '<span><img src="' +
        markIcon +
        '" class="optionTodo" id="mark" title="Mark as done"> <img src="delete_icon.png" class="optionTodo" id="delete" title="Delete"></span> </div>'
    );
  });
  /**
    ...
    $("#" + todoListMonths[index] + " .date").each(function () {
      ...
      if ($(this).html() == value) {
        fullDate =
          value +
          " " +
          todoListMonths[index].slice(0, 3) +
          ", " +
          currentYearName +
          " 00:00:00";
...
function redSundays() {
  ...
  $.each(yearArray[currentYearName], function (index, value) {
    value = 16 - value;
    $("#" + index + " .dates>div:nth-child(7n+" + value + ")").css(
      "color",
      "rgb(245, 182, 182)"
    );
  });
...
$(document).on("click", "#delete", function () {
  ...
  window.localStorage.setItem(
    currentYearStorage,
    JSON.stringify(pushEventDetail)
  );
  updateReminder();
  outlineEventDates();
});
...
$(document).on("click", "#mark", function () {
  ...
  $(this).attr(
    "src",
    $(this).attr("src") == "mark_icon.png" ? "unmark_icon.png" : "mark_icon.png"
  );
  $.each(alltodoList, function (index, value) {
    if (alltodoList[index].id == parentId) {
